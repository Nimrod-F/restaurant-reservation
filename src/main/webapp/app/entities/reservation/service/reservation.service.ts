...
export class ReservationService {
  ...
  protected convertDateFromClient(reservation: IReservation): IReservation {
    return Object.assign({}, reservation, {
      startDatetime: reservation.startDatetime?.isValid() ? reservation.startDatetime.toJSON() : undefined,
      endDateime: reservation.endDateime?.isValid() ? reservation.endDateime.toJSON() : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.endDateime = res.body.endDateime ? dayjs(res.body.endDateime) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((reservation: IReservation) => {
        reservation.startDatetime = reservation.startDatetime ? dayjs(reservation.startDatetime) : undefined;
        reservation.endDateime = reservation.endDateime ? dayjs(reservation.endDateime) : undefined;
      });
    }
    return res;
  }
